## `profile2consensus` {#refcmd-profile2consensus}

Extract consensus sequence DB from a profile DB.

### Classification

| Aspect | Value |
| :--- | :--- |
| API layer | `low_level_api` |
| Primary functional group | [`profiles`](#mod-profiles) |
| Category flags | `COMMAND_PROFILE` |

### Topology

| Aspect | Value |
| :--- | :--- |
| Upstream command count | `1` |
| Downstream command count | `0` |
| Workflow script count | `1` |
| Detailed dependency entry | [Open in map](#depcmd-profile2consensus) |

### Usage

No local help snapshot usage line is available.

### Key Options

No parsed options are available for this command.

### Full CLI Help Snapshot

```{=typst}
#doc_warning[
Help snapshot missing in mmseqs_help_output. Refresh local snapshots before relying on exact options/defaults.
]
```

### Cross References

See [Dependency map section](#sec-dependency-map), [dependency entry](#depcmd-profile2consensus), [command reference index](#sec-command-reference), and [functional module page](#mod-profiles).

