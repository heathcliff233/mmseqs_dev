### `transitivealign` {#refcmd-transitivealign}
...
Dependency entry: [Open in map](#depcmd-transitivealign); functional module: [`alignment`](#mod-alignment).

**Usage**

`usage: mmseqs transitivealign <inputDB(s)> <outputDB> [options]`

The syntax line above is source-derived from command layer/category metadata. Run `mmseqs transitivealign` locally for exact positional arguments in your build.
